...
CheckBrickCollision PROC
    ...
    ; --- Check if ball Y is inside brick zone ---
    mov ax, ballY
    cmp ax, brickTopY
    jb  NoBrickHit              ; ball above brick zone

    mov bx, brickTopY
    add bx, 64                  ; bottom of brick zone (4 rows * 16px)
    cmp ax, bx
    jae NoBrickHit              ; ball below brick zone

    ; row = (ballY - brickTopY) / brickRowH
    sub ax, brickTopY
    xor dx, dx
    mov bx, brickRowH
    div bx
    mov di, ax                  ; DI = row (0-3)

    ; --- Check if ball X is inside brick zone ---
    mov ax, ballX
    cmp ax, brickLeftX
    jb  NoBrickHit              ; ball left of brick zone

    mov bx, brickLeftX
    add bx, 288                 ; 8 cols * 36px
    cmp ax, bx
    jae NoBrickHit              ; ball right of brick zone

    ; col = (ballX - brickLeftX) / brickColW
    sub ax, brickLeftX
    xor dx, dx
    mov bx, brickColW
    div bx
    mov si, ax                  ; SI = col (0-7)
